...
## Status
Current session target: Phase 0 + Phase 1 + Phase 2 complete (M0-M5)
Last green commit: d660892 — "M5: extraction service — มคอ text -> ExtractedCourse draft via LLM, human-gate proof on real DT/PT specs ✓"
Tree state: ☑ green
...
- Building M6 (instructor form -> POST /api/session) and M7 (upload + correction
  screen) — these need the React frontend, so Node/npm must be available first. If
  Node still isn't installed, consider doing M8 (outline service, backend-only) ahead
  of schedule instead.
